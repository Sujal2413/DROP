...
export default function VariantShowcase() {
  return (
    <section id="products" className="w-full relative bg-[#F9F9F9] text-[#111111] font-inter border-b border-[#E5E5E5]/30 scroll-mt-24 overflow-hidden">
        ...
        {PRODUCTS.map((v) => (
          <div key={v.slug} id={`variant-${v.slug}`} className="flex flex-col md:flex-row items-center max-w-[1440px] mx-auto px-5 md:px-16 py-12 md:py-16 border-b border-[#E5E5E5]/30 scroll-mt-0">
            ...
            <div className="w-full md:w-1/2 flex flex-col justify-center items-start md:pl-24">
               ...
               {v.status === 'available' || v.status === 'preorder' || v.status === 'coming-soon' ? (
